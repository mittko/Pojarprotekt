...
import ServiceOrder.Renderers.ExtinguisherRenderer;
import ServiceOrder.Renderers.MyComboRenderer;
import ServiceOrder.Renderers.MyTableRenderer;
import ServiceOrder.ServiceModels.*;
import ServiceOrder.ServiceOrderWorkers.PrintPDFBarcodesWorker;
import ServiceOrder.ServiceOrderWorkers.PrinterServiceOrderWorker;
import ServiceOrder.ServiceOrderWorkers.SaveinServiceOrderDBWorker;
		...
		clientCombo.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent arg0) {
				...
				if (arg0.getItem().toString().length() > 0) {
					numberExt_Button.setEnabled(true);
				} else {
					numberExt_Button.setEnabled(false);
				}
			}
