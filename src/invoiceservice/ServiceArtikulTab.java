...
import invoiceservice.sklad.Worker.LoadAllArtikulsFromServiceInvoiceWorker;
...
public class ServiceArtikulTab extends MainPanel {
	...
	private final JTable оnlyArticulsTable;
	...
	public ServiceArtikulTab() {
		...
		skladButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				ServiceSkladArtikulFrame skladArtikulFrame = new ServiceSkladArtikulFrame(
						dftm/*
							 * , sumField
				...
				LoadAllArtikulsFromServiceInvoiceWorker loader = new LoadAllArtikulsFromServiceInvoiceWorker(
						new JDialog());
				loader.execute();
		...
		dbButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				SaveInServiceInvoiceDBDialog save = new SaveInServiceInvoiceDBDialog(INVOICE_PARENT,
						INVOICE_CHILD,
						"-",// here protokol
						// number is not
						// needed and is
		...
		оnlyArticulsTable = new JTable(dftm);
		// скрий колони фактура и контрагент
		оnlyArticulsTable.getColumnModel().getColumn(6).setMinWidth(0);
		оnlyArticulsTable.getColumnModel().getColumn(6).setMaxWidth(0);
		оnlyArticulsTable.getColumnModel().getColumn(6).setWidth(0);
		оnlyArticulsTable.getColumnModel().getColumn(7).setMinWidth(0);
		оnlyArticulsTable.getColumnModel().getColumn(7).setMaxWidth(0);
		оnlyArticulsTable.getColumnModel().getColumn(7).setWidth(0);

		оnlyArticulsTable.addMouseListener(new MouseAdapter() {
			@Override
			public void mousePressed(MouseEvent me) {
				Point point = me.getPoint();
				int currentRow = оnlyArticulsTable.rowAtPoint(point);
				оnlyArticulsTable.setRowSelectionInterval(currentRow,
						currentRow);
			}
		});
		оnlyArticulsTable.setDefaultRenderer(Object.class,
				new CustomTableCellRenderer(оnlyArticulsTable));
		оnlyArticulsTable.setRowHeight(MainPanel.getFontSize() + 15);
		...
		menuItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				dftm.removeRow(оnlyArticulsTable.getSelectedRow());
				if (dftm.getRowCount() == 0) {
					clear();
				}
				//   else {
		...
		оnlyArticulsTable.setComponentPopupMenu(popupMenu);

		JScrollPane scroll = new JScrollPane(оnlyArticulsTable,
