...
public class InvoicePDFFromReports extends PdfCreator {
	...
	public boolean createInvoicePDF(ArrayList<String> clientInfo, String num,
		...
		for (row = 0; dftm != null && row < stop; row++) {
			...
			String obslujvane = dftm.getValueAt(row + startIndex, 10).toString();
			/*
	...
	public boolean createInvoicePDF(ArrayList<String> clientInfo, String num,
		...
		float danOsnova = 0;
			...
			PdfPCell measureCell = new PdfPCell(new Phrase("1", arial10));// measure
			measureCell.setHorizontalAlignment(Element.ALIGN_CENTER);

			PdfPCell quantityCell = new PdfPCell(new Phrase("1", arial10));
			quantityCell.setHorizontalAlignment(Element.ALIGN_RIGHT);
