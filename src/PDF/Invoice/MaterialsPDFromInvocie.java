...
public class MaterialsPDFromInvocie extends PdfCreator {
	...
	public void createMaterialsPDF(ArrayList<String> clientInfo2,
			DefaultTableModel dftm, String timeStamp, String invoiceNumber,
		...
		try {
			pdfWriter = PdfWriter.getInstance(document, new FileOutputStream(
					MainPanel.MATERIALS_PDF_PATH + "\\Вложени Материали-"
							+ timeStamp + "-" + invoiceNumber + ".pdf"));
		...
		setText("Вложени Материали към Фактура \u2116 " + invoiceNumber, titleX, titleY,
				"arialbd", 12);
		...
		setMainTable(dftm, upTableX,
				...
				endIndex, finalSumIndex, pdfWriter);
...
//
//		// repeat same table
		setText("Вложени Материали към Фактура № " + invoiceNumber, titleX,
				y2 - 10, "arialbd", 12);
//
		...
		setMainTable(dftm, x2, y2 - (upTable.getTotalHeight() + 30),
				startIndex, endIndex, finalSumIndex, pdfWriter);
	...
	public static void main(String[] args) {
		MaterialsPDFromInvocie pdf = new MaterialsPDFromInvocie();
		String timeStamp2 = MyGetDate.getTimeStamp();
		pdf.createMaterialsPDF(new ArrayList<String>(),
