...
public class MaterialsPDFromReports extends PdfCreator {
	...
	public void createMaterialsPDF(ArrayList<String> clientInfo2,
		...
		setText("Вложени Материали към Фактура \u2116 "
	...
	private void setMainTable(DefaultTableModel dftm, float mainTableX,
		...
		for (int row = 0; row < endIndex; row++) {
			...
			String doing = dftm.getValueAt(row + startIndex, 10).toString();
			...
			String quantity = (dftm.getValueAt(row + startIndex, 12).toString());
			...
			PdfPCell pricCell = new PdfPCell(new Phrase(dftm.getValueAt(
					row + startIndex, 13).toString(), arial10));
			pricCell.setHorizontalAlignment(Element.ALIGN_CENTER);

			mainTable.addCell(pricCell);

			String sumOfPrices = dftm.getValueAt(row + startIndex, 14)
					.toString();
	...
	public static void main(String[] args) {
		MaterialsPDFromReports pdf = new MaterialsPDFromReports();
