...
public class GreySkladArtikulFrame extends MainPanel {
	...
	public GreySkladArtikulFrame(final DefaultTableModel invoiceTableModel
	/*
	 ...
	 */, final double clientDiscount, final boolean isDiscountSelect) {
		...
		skladTable.getColumnModel().getColumn(4)
				.setCellEditor(new AcquittanceTableCellEditor(helpTextField));

		skladTable.setDefaultRenderer(Object.class, new AcquittanceAutoSortRenderer(
				skladTable));
...
//						invoiceByKontragent });  OLD LOGIC THAT WORK !!!
	...
	private void goodbyeCruelWorld() {
		JDialog jd = (JDialog) SwingUtilities
				.getWindowAncestor(GreySkladArtikulFrame.this);
		jd.dispose();
	}
